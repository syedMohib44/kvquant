...
% -----------------------------------------------------------------------
\begin{abstract}
TurboQuant \citep{zandieh2025} is a compelling approach to KV cache
...
We introduce five extensions---attention-weighted quantization, delta
compression, adaptive bit allocation, low-rank error correction, and
product quantization---each targeting a different structural property of
transformer attention that the original method leaves on the table. Along
the way, we also corrected several issues in the original implementation: the
codebook was fitted to a Gaussian approximation rather than the actual sphere
marginal distribution; the QR decomposition could silently produce a
reflection instead of a rotation; the nearest-centroid search was doing
$O(N \cdot d \cdot k)$ work when a binary search suffices; and the
inner-product quantizer was applying a redundant second normalisation pass on
vectors already on the unit sphere.
...
extra storage. Product quantization ($M{=}16$, $b{=}8$) produces coherent
generation at 2 bits/dim---the same storage as 2-bit scalar, which collapses
matching 3-bit scalar quality. The \texttt{bucketize} lookup runs 14--22$\times$
faster than the original \texttt{argmin} expansion. Four additional
improvements are described: k-means++ codebook initialisation (75\% lower
init MSE at 1-bit), K-V asymmetric quantization (V MSE reduced 61.5\% at
0.5 fewer bits/dim), delta+outlier combination (V MSE reduced 95.4\% vs
same-budget plain), and Hadamard rotation exposed as a configurable parameter
($O(d \log d)$ vs $O(d^2)$).

Separately from the extensions, we make the compression \emph{real}: the
shipped cache stores compact codes rather than decompressing to float, so
quantization now reduces memory instead of only simulating its quality cost.
On Qwen2.5-0.5B-Instruct this holds 2.5--3.6$\times$ fewer cache bytes than
fp16 with per-vector norm sidecars counted, and cuts peak VRAM by up to
1.9$\times$ at $\sim$14k tokens at equal throughput---a saving that grows
with context and is absent below $\sim$1k tokens, where the prefill attention
matrix rather than the cache sets the peak (Section~5.4).

Two caveats stated up front, because both were previously overstated. First,
our measured compression ratios are roughly \textbf{half} the nominal $16/b$
figure, because that figure ignores the per-vector L2 norms the codec must
store; we count them. Second, each extension is measured \textbf{in
isolation}, against its own baseline. An earlier version of this paper claimed
the gains ``compound'' when composed; that claim was never measured, is
contradicted for one pair of stages by our own Section~3.3, and is withdrawn
in Section~4.
\end{abstract}
...
The paper is organized around these five extensions, preceded by a
description of the implementation improvements we made to the baseline. Each
extension works independently and is measured independently; whether they
stack is examined, and left open, in Section~4.

There is also a problem that is not about compression ratios at all. A KV
cache quantizer that compresses and then immediately decompresses to float
reproduces quantization's \emph{quality cost} while banking none of its
\emph{memory benefit} --- and that is what our own implementation did until
recently, and what a surprising amount of published work measures.
Section~5.4 reports what the cache actually holds, and Section~5.5 states how
it is measured, because a compression result whose ruler is unspecified is
not reproducible.
...
\subsubsection{GQA Compensation and Per-Layer Calibration}

\paragraph{GQA amplification.}
Models with Grouped Query Attention (GQA) share each KV head across
$g = n_{\text{heads}} / n_{\text{kv-heads}}$ query heads. Because the same
quantized KV vector is used in $g$ independent attention computations, the
effective per-head distortion is amplified:
\begin{equation}
  D_{\mathrm{eff}} \;\approx\; g \cdot D_{\mathrm{mse}}.
  \label{eq:gqa_amp}
\end{equation}
For Qwen2.5-1.5B ($g=6$, $d=128$) even 4-bit quantization
($D_{\mathrm{mse}} \leq 0.011$ from Eq.~\ref{eq:mse_bound}) gives
$D_{\mathrm{eff}} \approx 0.066$, which is large enough to corrupt
generation.

\paragraph{Compensation formula.}
We solve for the effective bit-width $b_{\mathrm{eff}}$ such that the
amplified distortion of the GQA model matches the target of a standard
$b$-bit MHA model:
\begin{equation}
  g \cdot \frac{\sqrt{3}\,\pi}{2} \cdot 4^{-b_{\mathrm{eff}}}
  \;=\; \frac{\sqrt{3}\,\pi}{2} \cdot 4^{-b}
  \quad\Rightarrow\quad
  b_{\mathrm{eff}} \;=\; b + \log_4 g \;=\; b + \frac{\log g}{\log 4}.
\end{equation}
Since bit-widths must be integers we round up:
$b_{\mathrm{eff}} = b + \lceil \log_4 g \rceil$.
Concretely: $g=4$ adds $+1$ bit; $g=6$ or $g=7$ adds $+2$ bits;
$g=32$ adds $+3$ bits. Effective bits are capped at 8 (the maximum supported
by the Lloyd-Max solver with 256 centroids). The same adjustment is applied
to the outlier and regular channel bit-widths in \texttt{OutlierKVQuant} so
that GQA compensation cannot be bypassed by caller-supplied explicit
bit-widths.

\begin{lstlisting}[language=Python]
# KVCacheQuantizer.__init__  (kv_cache.py)
gqa_extra = math.ceil(math.log(max(gqa_factor, 1), 4)) \
            if gqa_factor > 1 else 0
effective_bits = min(num_bits + gqa_extra, 8)

# GQA extra applied to outlier/regular bits as well
ob = min(ob_base + gqa_extra, 8)
rb = min(max(rb_base + gqa_extra, 1), 8)
\end{lstlisting}

\paragraph{Per-layer calibration.}
\texttt{OutlierKVQuant} identifies outlier channels as those with the
highest variance in the calibration data. Different transformer layers have
completely different KV distributions---layer~0 may concentrate variance in
dimensions $\{12, 47, 93\}$ while layer~15 concentrates it in
$\{5, 61, 120\}$. Using a single quantizer calibrated on data pooled from
all layers averages out these patterns and causes systematic misidentification
of outlier channels in every individual layer.

The correct approach is one \texttt{KVCacheQuantizer} per transformer layer,
each calibrated independently on its own layer's KV data:
...
This is consistent with the paper's framework: the Lloyd-Max quantization
theory is unchanged, and only the channel-identification step is made
layer-specific. For MHA models with similar KV distributions across layers
a single shared quantizer remains acceptable. For GQA models with deep
per-layer specialisation, per-layer calibration is essential.
...
$b=4$, $d=128$ the entropy is 3.771 bits vs.\ 4 raw bits, giving roughly 4\%
...
\texttt{kvquant/}, and they can be adopted in any combination.

We also addressed two implementation issues specific to non-MHA architectures.
GQA models amplify effective distortion by a factor of $g$ (the number of
query heads per KV head); compensating with $\lceil \log_4 g \rceil$ extra
bits per coordinate, capped at 8, restores generation quality on models such
as Qwen2.5-1.5B ($g=6$) and Qwen2.5-7B ($g=7$). Per-layer calibration of
the outlier detector, rather than pooling KV data across all transformer
layers, correctly identifies the layer-specific channels that carry anomalous
variance. Both fixes are active when \texttt{gqa\_factor} $> 1$ is passed to
\texttt{KVCacheQuantizer}. The full test suite of 88 unit tests passes cleanly.
